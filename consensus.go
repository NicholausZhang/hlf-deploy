...
func updateConsensusState(_ *cobra.Command, args []string) {
	...
	// get modified config
	modifiedConfigBytes := utils.GetChannelConsensusStateModifiedConfig(configBytes,
		utils.GetConsensusState(consensusState), consensusType,
		consensusOptionElectionTick, consensusOptionHeartbeatTick, consensusOptionMaxInflightBlocks,
		consensusOptionSnapshotIntervalSize, consensusOptionTickInterval, sysChannel)
	...
	log.Printf("update %s channel consensus status to %s success txID: %s", channelName, consensusState, txID.TransactionID)
}
